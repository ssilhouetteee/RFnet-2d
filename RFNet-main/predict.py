...
def softmax_output_dice_class4(output, target):
    ...
    o1 = (output == 1).float()
    t1 = (target == 1).float()
    intersect1 = torch.sum(2 * (o1 * t1), dim=(1,2)) + eps
    denominator1 = torch.sum(o1, dim=(1,2)) + torch.sum(t1, dim=(1,2)) + eps
    ncr_net_dice = intersect1 / denominator1

    o2 = (output == 2).float()
    t2 = (target == 2).float()
    intersect2 = torch.sum(2 * (o2 * t2), dim=(1,2)) + eps
    denominator2 = torch.sum(o2, dim=(1,2)) + torch.sum(t2, dim=(1,2)) + eps
    edema_dice = intersect2 / denominator2

    o3 = (output == 3).float()
    t3 = (target == 3).float()
    intersect3 = torch.sum(2 * (o3 * t3), dim=(1,2)) + eps
    denominator3 = torch.sum(o3, dim=(1,2)) + torch.sum(t3, dim=(1,2)) + eps
    enhancing_dice = intersect3 / denominator3

    ####post processing:
    # 2D 情况下 region 像素数量会明显小于 3D，所以这里用相对阈值代替固定 500
    min_pixels = o3.shape[0] * o3.shape[1] * o3.shape[2] * 0.01
    if torch.sum(o3) < min_pixels:
       o4 = o3 * 0.0
    else:
       o4 = o3
    t4 = t3
    intersect4 = torch.sum(2 * (o4 * t4), dim=(1,2)) + eps
    denominator4 = torch.sum(o4, dim=(1,2)) + torch.sum(t4, dim=(1,2)) + eps
    enhancing_dice_postpro = intersect4 / denominator4

    o_whole = o1 + o2 + o3 
    t_whole = t1 + t2 + t3 
    intersect_whole = torch.sum(2 * (o_whole * t_whole), dim=(1,2)) + eps
    denominator_whole = torch.sum(o_whole, dim=(1,2)) + torch.sum(t_whole, dim=(1,2)) + eps
    dice_whole = intersect_whole / denominator_whole

    o_core = o1 + o3
    t_core = t1 + t3
    intersect_core = torch.sum(2 * (o_core * t_core), dim=(1,2)) + eps
    denominator_core = torch.sum(o_core, dim=(1,2)) + torch.sum(t_core, dim=(1,2)) + eps
...
def softmax_output_dice_class5(output, target):
    ...
    ####post processing:
    min_pixels = o4.shape[0] * o4.shape[1] * o4.shape[2] * 0.01
    if torch.sum(o4) < min_pixels:
        o5 = o4 * 0
    else:
        o5 = o4
    t5 = t4
    intersect5 = torch.sum(2 * (o5 * t5), dim=(1,2)) + eps
    denominator5 = torch.sum(o5, dim=(1,2)) + torch.sum(t5, dim=(1,2)) + eps
