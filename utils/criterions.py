...
def dice_loss(output, target, num_cls=5, eps=1e-7):
    ...
    for i in range(num_cls):
        # output/target: [B, C, H, W] -> keep scalar (same behavior as original 3D code)
...
def softmax_weighted_loss(output, target, num_cls=5):
    ...
    B, _, H, W = output.size()
    for i in range(num_cls):
        outputi = output[:, i, :, :]   # [B, H, W]
        targeti = target[:, i, :, :]   # [B, H, W]
        weighted = 1.0 - (torch.sum(targeti, (1, 2)) * 1.0 / torch.sum(target, (1, 2, 3)))
        weighted = weighted.view(B, 1, 1).repeat(1, H, W)  # [B, H, W]
        if i == 0:
