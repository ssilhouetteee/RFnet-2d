...
#####################################################################################
parser.add_argument('--resume_train', default=None, type=str) # <--- 新加这行
#####################################################################################
...
setup(args, 'training')
args.train_transforms = 'Compose([NumpyType((np.float32, np.int64)),])'
...
#############################################################################################
    ...
    if args.resume_train is not None:
        ...
        start_epoch = checkpoint['epoch'] + 1 # 提取记录的轮数，并加1（从下一轮接着跑）
        model.load_state_dict(checkpoint['state_dict']) # 恢复模型记忆
        optimizer.load_state_dict(checkpoint['optim_dict']) # 恢复优化器节奏
        logging.info('顺利恢复训练！将从第 {} 轮接着跑...'.format(start_epoch))
###############################################################################################  
    ...
    for epoch in range(start_epoch, args.num_epochs): # <--- 把 start_epoch 加进去
